...
class ReleasePolicyTests(unittest.TestCase):
    ...
    def test_release_workflow_can_create_draft_from_dry_run_artifact(self):
        ...
        self.assertIn("CFBundleShortVersionString", reuse_section)
        self.assertIn("MacActivityReleaseTag", reuse_section)
        self.assertIn("MacActivityBundleBuild", reuse_section)
        self.assertIn("Applications Finder alias", reuse_section)
    ...
    def test_styled_dmg_uses_hidden_background_and_finder_layout(self):
        ...
        self.assertIn("DMG_HEADROOM_MEGABYTES=16", script)
        self.assertIn('STAGING_SIZE_MEGABYTES="$(du -sm "${STAGING_DIR}"', script)
        self.assertIn("RW_DMG_SIZE_MEGABYTES=\"$((STAGING_SIZE_MEGABYTES + DMG_HEADROOM_MEGABYTES))\"", script)
        self.assertIn("-size", script)
        self.assertIn('"${RW_DMG_SIZE_MEGABYTES}m"', script)
        self.assertIn("-format UDRW", script)
        self.assertIn("detach_mount", script)
        self.assertIn('hdiutil detach "${mount_point}" -force', script)
