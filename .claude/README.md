...
plugin so there is one source of truth for PR and release workflows.
...
- `mac-activity:create-release`
- `mac-activity:pr-readiness-check`

The `SKILL.md` frontmatter names stay unprefixed (`create-pr`,
`create-release`, `pr-readiness-check`) because Claude validates them against
the linked folder names. The plugin provides the `mac-activity:` namespace.
